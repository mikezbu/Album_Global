...
export default class TrendingSampleStore extends Paginator {
  ...
  constructor(initialData: TrendingSampleStore = null) {
    super()
    ...
    if (initialData) {
      if (initialData.samples) {
        ...
        const samples = observable.map<number, SampleModel>()

        Object.values(initialData.samples).forEach((element: any) => {
          if (element && element[1] && element[1].id) {
            samples.set(element[1].id, new SampleModel(element[1]))
            sampleIds.push(element[1].id)
          } else if (element && element.id) {
            samples.set(element.id, new SampleModel(element))
